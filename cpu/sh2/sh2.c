...
{
	sh2->pc = p32x_soc_read32(0, sh2);
	sh2->r[15] = p32x_soc_read32(4, sh2);
	sh2->sr = I;
	sh2->vbr = 0;
...
void sh2_do_irq(SH2 *sh2, int level, int vector)
{
	...
	sh2->r[15] -= 4;
	p32x_soc_write32(sh2->r[15], sh2->sr, sh2);	/* push SR onto stack */
	sh2->r[15] -= 4;
	p32x_soc_write32(sh2->r[15], sh2->pc, sh2);	/* push PC onto stack */

	/* set I flags in SR */
	sh2->sr = (sh2->sr & ~I) | (level << 4);

	/* fetch PC */
	sh2->pc = p32x_soc_read32(sh2->vbr + vector * 4, sh2);
